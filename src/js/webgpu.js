...
export const webGPUNativeImports = {
    ...
    env_wgpu_poll_promise_js: function(promise_id) {
        ...
        // Check actual promise state if it's a live promise
        // This is a simplified polling model. Real applications might need more robust handling.
        // For this FFI, we assume the promise has been processed by the event loop
        // and its .then() or .catch() has replaced it with a status object.
        // If it's still a Promise object, it's pending.
        if (typeof promise_entry.then === 'function') {
             // To make it eventually resolve for polling, we can re-wrap it here,
             // but ideally, the initial storePromise already handles this by
             // storing the result of .then/.catch.
             // This current logic assumes that if it's not a status object, it's pending.
            return 0;
        }
        // If it's not a known status and not a promise, it's an unknown state.
        // However, given the structure, it should always be one of the above.
        // The `storePromise` logic ensures that what's stored is the result of the `.then().catch()` chain.
        return 0; // Default to pending if not explicitly fulfilled or rejected.
    },
    ...
    env_wgpu_get_adapter_from_promise_js: function(promise_id) {
        ...
        if (result && result.status === 'rejected') {
            globalWebGPU.error = result.error; // Store error for Zig to fetch
        }
    ...
    env_wgpu_get_last_error_msg_ptr_js: function() {
        if (globalWebGPU.error) {
            ...
            // globalWebGPU.error = null; // Clear after encoding for Zig
            return 1; // Non-null address, indicates an error is available (Zig will use len)
        }
        lastErrorBytes = null;
        return 0; // Null address, indicates no error
    },
    ...
    env_wgpu_copy_last_error_msg_js: function(buffer_ptr, buffer_len) {
        if (lastErrorBytes && buffer_ptr && buffer_len > 0) {
            ...
            if (!memory) {
                ...
                // To prevent Zig from reading garbage if it proceeds:
                const tempError = "Wasm memory not available to JS for error reporting.";
                const encoder = new TextEncoder();
                lastErrorBytes = encoder.encode(tempError); // Update lastErrorBytes
                // Do not clear globalWebGPU.error, so it can be logged by JS if needed.
                return;
            }
            ...
            // if Zig requests less than the full length, it's up to Zig to call again or manage.
            // For simplicity, clearing after any copy attempt.
            lastErrorBytes = null;
            globalWebGPU.error = null; // Clear the original error source as well
        }
    },
    ...
    js_log_string: function(message_ptr, message_len) {
        if (!this.wasmMemory) {
            ...
            // Fallback log if memory isn't set up, but message is lost
